...
async function main() {
  ...
  const tags = await Promise.all([
    prisma.tag.upsert({ where: { slug: 'exploitation' },            create: { name: 'Exploitation', slug: 'exploitation' },                       update: {} }),
    prisma.tag.upsert({ where: { slug: 'class' },                   create: { name: 'Class', slug: 'class' },                                     update: {} }),
    prisma.tag.upsert({ where: { slug: 'historical-materialism' },  create: { name: 'Historical Materialism', slug: 'historical-materialism' },    update: {} }),
    prisma.tag.upsert({ where: { slug: 'justice' },                 create: { name: 'Justice', slug: 'justice' },                                 update: {} }),
    prisma.tag.upsert({ where: { slug: 'game-theory' },             create: { name: 'Game Theory', slug: 'game-theory' },                         update: {} }),
    prisma.tag.upsert({ where: { slug: 'rational-choice' },         create: { name: 'Rational Choice', slug: 'rational-choice' },                 update: {} }),
    prisma.tag.upsert({ where: { slug: 'basic-income' },            create: { name: 'Basic Income', slug: 'basic-income' },                       update: {} }),
    prisma.tag.upsert({ where: { slug: 'methodology' },             create: { name: 'Methodology', slug: 'methodology' },                         update: {} }),
  ]);

  const tagMap = Object.fromEntries(tags.map(t => [t.slug, t]));

  // ─── Thinkers ──────────────────────────────────────────
  const cohen = await prisma.thinker.upsert({
    where: { slug: 'ga-cohen' },
    update: {},
    create: {
      slug: 'ga-cohen',
  ...
  const elster = await prisma.thinker.upsert({
    where: { slug: 'jon-elster' },
    update: {},
    create: {
      slug: 'jon-elster',
      name: 'Jon Elster',
  ...
  const roemer = await prisma.thinker.upsert({
    where: { slug: 'john-roemer' },
    update: {},
    create: {
      slug: 'john-roemer',
      name: 'John Roemer',
  ...
  const wright = await prisma.thinker.upsert({
    where: { slug: 'erik-olin-wright' },
    update: {},
    create: {
  ...
  const vanParijs = await prisma.thinker.upsert({
    where: { slug: 'philippe-van-parijs' },
    update: {},
    create: {
  ...
  const przeworski = await prisma.thinker.upsert({
    where: { slug: 'adam-przeworski' },
    update: {},
    create: {
  ...
  await prisma.post.upsert({
    where: { slug: 'what-is-analytical-marxism' },
    update: {},
    create: {
      ...
      tags: {
        connect: [
          { id: tagMap['methodology'].id },
          { id: tagMap['rational-choice'].id },
        ],
      },
    },
  });
  ...
  await prisma.post.upsert({
    where: { slug: 'exploitation-without-labor-theory-of-value' },
    update: {},
    create: {
      ...
      tags: {
        connect: [
          { id: tagMap['exploitation'].id },
          { id: tagMap['game-theory'].id },
        ],
      },
    },
  });
  ...
  await prisma.post.upsert({
    where: { slug: 'ai-automation-and-wrights-real-utopias' },
    update: {},
    create: {
  ...
  await prisma.post.upsert({
    where: { slug: 'reading-cohens-karl-marxs-theory-of-history' },
    update: {},
    create: {
      ...
      content: `## Why This Book Matters\n\nPublished in 1978, *Karl Marx's Theory of History: A Defence* (KMTH) is the founding text of Analytical Marxism. Cohen set himself an ambitious task: to state Marx's theory of history with enough precision that it could be properly evaluated — defended where defensible, and rejected where not.\n\nThe result was a landmark in both Marxist theory and analytic philosophy.\n\n## The Core Argument\n\nCohen's reconstruction of historical materialism rests on two central theses:\n\n**The Development Thesis:** The productive forces (technology, skills, knowledge) tend to develop throughout history.\n\n**The Primacy Thesis:** The nature of the production relations (property relations, class structure) is explained by the level of development of the productive forces.\n\nThe second thesis is the controversial one. Cohen argues that it involves *functional explanation*: the relations of production are as they are *because* they are optimal for the development of the productive forces at a given stage. When the forces outgrow the existing relations, revolutionary transformation occurs.\n\n## The Philosophical Innovation\n\nCohen's key philosophical contribution was to show that functional explanation — explaining something by its consequences — is legitimate in social science, provided that an underlying causal mechanism can in principle be specified. He drew on examples from biology (natural selection as a mechanism for functional traits) to argue that functional explanation is not inherently teleological or mystical.\n\n## How to Read It\n\nThe book rewards careful, chapter-by-chapter reading. Start with the Preface for Cohen's intellectual autobiography, then work through Part I (chapters 1–6) for the core reconstruction. Part II addresses objections and alternative interpretations. The expanded edition (2000) includes valuable additional essays responding to critics.`,
      category: 'reading',
      difficulty: 'intermediate',
      published: true,
      featured: false,
      ...
      tags: {
        connect: [
          { id: tagMap['historical-materialism'].id },
          { id: tagMap['methodology'].id },
        ],
      },
    },
  ...
  await prisma.infographic.upsert({
    where: { slug: 'september-group-timeline' },
    update: {},
    create: {
  ...
  await prisma.infographic.upsert({
    where: { slug: 'roemers-exploitation-game' },
    update: {},
    create: {
  ...
  await prisma.infographic.upsert({
    where: { slug: 'wrights-class-typology' },
    update: {},
    create: {
